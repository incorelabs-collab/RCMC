var app = {
    ...
    getAppDb: function() {
        ...
        return openDatabase('dbRcmc', localStorage.getItem('dbLocalVersion'), 'dbRcmc', (5 * 1022 * 1022));
    },
    getImgDb: function() {
        if(localStorage.getItem('imgDbLocalVersion') == null) {
            localStorage.setItem('imgDbLocalVersion','-1');
        }
        return openDatabase('imgDbRcmc', localStorage.getItem('imgDbLocalVersion'), 'imgDbRcmc', (5 * 1022 * 1022));
    },
    initialize: function() {
    ...
    doOnlineTasks: function() {
        var urlData = 'http://rcmc.incorelabs.com/db_version.php';
    ...
    checkWithLocalDB: function(json) {
        if (localStorage.getItem("dbLocalVersion") != json[0][0]) {
            ...
            $.getJSON('http://rcmc.incorelabs.com/male.php', function(maleData) {
                app.createTable(maleData,"male",1);
            });
            $.getJSON('http://rcmc.incorelabs.com/female.php', function(femaleData) {
                app.createTable(femaleData,"female",2);
            });
            $.getJSON('http://rcmc.incorelabs.com/common.php', function(commonData) {
                app.createTable(commonData,"common",3);
            });
            $.getJSON('http://rcmc.incorelabs.com/kids.php', function(kidsData) {
                app.createTable(kidsData,"kids",4);
            });
            $.getJSON('http://rcmc.incorelabs.com/directors.php', function(directorsData) {
    ...
    getImageAssets: function () {
        var urlImages = 'http://rcmc.incorelabs.com/images/file-list.php?key=kamlesh';
    ...
    getSecondAd: function() {
        var urlImages = 'http://rcmc.incorelabs.com/adTwo/file-list.php?key=kamlesh';
        var dirReference = app.getDirectoryReference();
        dirReference.done(function(imgDir) {
            app.imgDir = imgDir;
